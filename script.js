...
const DAYS = [
  ...
  /* ---- Day 2: 7/5 Zurich → Interlaken ---- */
  {
    ...
    tags: ['🛬 チューリッヒ到着', '🚆 SBB鉄道', '🏔 インターラーケン', '🚠 ハーダークルム夕景', '🏨 レジデンス ユングフラウ'],
    schedule: {
      '早朝 — 乗継': [
      ...
      '午前〜午後 — 散策': [
        ...
        { time: '15:30', icon: '🏨', name: 'ホテル チェックイン', desc: 'レジデンス ユングフラウ（Seestrasse 10, Interlaken）チェックイン 16:00〜21:30。本格的な観光は明日から。今夜は早めに休む。' },
      ],
      '夕方 — ハーダークルム': [
        ...
        { time: '22:00', icon: '🏨', name: 'ホテル', desc: 'レジデンス ユングフラウ泊（インターラーケン）' },
      ]
    }
  },
  /* ---- Day 3: 7/6 Bernese Oberland ---- */
  {
    ...
    tags: ['⛰ グリンデルワルト', '❄️ ユングフラウヨッホ', '💦 ラウターブルンネン', '🏨 レジデンス ユングフラウ'],
    schedule: {
      '早朝': [
      ...
      '午後 — ラウターブルンネン': [
        ...
        { time: '21:00', icon: '🏨', name: 'ホテル', desc: 'レジデンス ユングフラウ泊。明日は早起き！チェックアウトは翌朝 07:00〜10:00。' },
      ]
    }
  },
  /* ---- Day 4: 7/7 Swiss → Venice ---- */
  {
    ...
    schedule: {
      ...
      '夕方 — ベネチア': [
        { time: '16:30', icon: '🚌', name: 'ホテルへ移動', desc: 'バス or タクシーでメストレのホテルへ（Hotel alla Giustizia / Via Miranese 111）。空港からメストレへはバスで約30分。' },
        { time: '17:00', icon: '🏨', name: 'チェックイン', desc: 'Hotel alla Giustizia チェックイン（CI 15:00〜23:30）。ベネチア本島へは鉄道・バスで約15分。※PIN: 4310' },
  ...
  /* ---- Day 5: 7/8 Venice ---- */
  {
    ...
    tags: ['⛪ サン・マルコ寺院', '🏛 ドゥカーレ宮殿', '🚤 ゴンドラ', '🛒 リアルト市場', '🏨 Hotel alla Giustizia'],
    schedule: {
      '午前': [
  ...
  /* ---- Day 6: 7/9 Venice → Rome ---- */
  {
    ...
    tags: ['🚄 Italo 予約済み', '🏛 ローマ到着', '🏛 パンテオン', '⛲ トレビの泉', '🏨 Terrazza Munira Trastevere'],
    schedule: {
      '午前 — 移動': [
  ...
  /* ---- Day 7: 7/10 Rome Colosseum + Vatican ---- */
  {
    ...
    tags: ['🏟 コロッセオ ✅予約済', '⛪ バチカン ✅予約済', '🎨 システィーナ礼拝堂', '🏨 Terrazza Munira Trastevere'],
    schedule: {
      '午前 — 古代ローマ': [
      ...
      '昼 — ゆったり移動': [
        { time: '13:00', icon: '🍝', name: '昼食', desc: 'コロッセオ周辺のトラットリアでゆっくり昼食。バチカン入場は16:00なので時間に余裕あり' },
        { time: '14:30', icon: '🚌', name: 'バチカンへ移動', desc: 'バス or 地下鉄（A線 Ottaviano 駅）で約30分' },
        { time: '15:15', icon: '🕊', name: 'サン・ピエトロ広場 散策', desc: 'ベルニーニの大列柱廊に囲まれた広場を先に散策。入場口（CORRIDOIO 1）の場所を確認しておく' },
        { time: '15:45', icon: '⏳', name: '入場口へ', desc: 'CORRIDOIO 1 / PASSAGEWAY 1 の入場口に並ぶ。チケット（印刷 or スマホ）＋身分証を用意' },
      ],
      '午後 — バチカン': [
        { time: '16:00', icon: '🎨', name: 'ヴァチカン美術館 入場', desc: '入口はCORRIDOIO 1（PASSAGEWAY 1）。入場前に廊下でスタッフによるチケット確認が必須。閉館18:00', note: '✅ 16:00 予約済' },
        { time: '17:30', icon: '🎨', name: 'システィーナ礼拝堂', desc: '「最後の審判」と天井画に圧倒される。美術館ルートの終盤に位置する。写真撮影不可' },
        { time: '18:00', icon: '⛪', name: 'サン・ピエトロ大聖堂', desc: '美術館退出後そのままバジリカへ（徒歩5分・無料）。夕暮れ時の荘厳な内部を堪能。19:00頃閉堂' },
      ],
      '夜': [
        { time: '19:30', icon: '🍷', name: '夕食', desc: 'テヴェレ川沿いまたはトラステヴェレのトラットリアへ。ホテルのあるトラステヴェレは徒歩20分 or バス' },
        { time: '21:00', icon: '🏨', name: 'ホテル', desc: 'Terrazza Munira Trastevere 泊（ローマ）' },
      ]
    }
  },
  /* ---- Day 8: 7/11 Last day Rome + Departure ---- */
  {
    ...
    schedule: {
      '午前': [
        { time: '08:30', icon: '🍳', name: '朝食・チェックアウト準備', desc: 'Terrazza Munira Trastevere にて朝食。チェックアウト（CO 〜10:00）後、荷物をホテルに預けて観光へ。' },
      ...
      '午後 — 帰国': [
        { time: '15:30', icon: '🏨', name: 'Terrazza Munira Trastevere 荷物回収', desc: 'ホテルに預けておいたスーツケースを回収して空港へ向かう。' },
